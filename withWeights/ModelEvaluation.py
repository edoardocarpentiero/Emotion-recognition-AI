...
os.makedirs("../results/plots/withWeights", exist_ok=True)
...
plt.tight_layout()
plt.savefig('../results/plots/withWeights/confusion_matrix.png', dpi=300)
plt.show()
...
os.makedirs("../results/report/withWeights", exist_ok=True)
with open('../results/report/withWeights/classification_report.txt', 'w', encoding='utf-8') as f:
    f.write(report)
...
plt.tight_layout()
plt.savefig('../results/plots/withWeights/top_confused_classes_heatmap.png', dpi=300)
plt.show()
...
with open('../results/report/withWeights/top_confused_classes.txt', 'w', encoding='utf-8') as f:
    for err, count in sorted_errors:
        f.write(f"{err}: {count} volte\n")

# === Salva le immagini sbagliate in cartelle separate ===
os.makedirs("../results/errors/withWeights", exist_ok=True)
...
for i, (true, pred) in enumerate(zip(true_labels, predicted_classes)):
    if true != pred:
        ...
        error_dir = os.path.join("../results/errors/withWeights", f"{true_class}_pred_{pred_class}")
