...
plt.title("Top confusioni di classe (pre-bilanciamento)")
plt.ylabel("Classe vera")
plt.xlabel("Classe predetta")
plt.tight_layout()
plt.savefig('plots/top_confused_classes_heatmap.png', dpi=300)
plt.show()
...
with open('results/top_confused_classes.txt', 'w', encoding='utf-8') as f:
...
for i, (true, pred) in enumerate(zip(true_labels, predicted_classes)):
    if true != pred:
        ...
        error_dir = os.path.join("errors", f"{true_class}_pred_{pred_class}")
